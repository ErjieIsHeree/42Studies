...
/*   Updated: 2026/05/28 11:54:56 by exia             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	destroy_sim(t_sim *sim, t_error error)
{
	...
	if (sim->dongles)
		free(sim->dongles);
	if (!error || error > FINISHED_MUTEX_INIT_ERROR)
...
int	codex_destroy(t_sim *sim, t_error error, int delete_size)
{
	...
	else if (!error || error > CODER_MUTEX_INIT_ERROR)
